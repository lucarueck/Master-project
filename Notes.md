...
- **FairSTG**: 
    ...
    - Bias-generating attributes can be unknown a-priori
    - contains traffic and electricity datasets
    - ensures fairness between individuals or cities not between groups (therefore the variance is taken as the metric)
...
- **SA-Net**: 
    ...
    - Chicago TNC dataset is easily accesible
    - fairness between regions should be ensured


- **STEER**:
    - https://github.com/weiliao97/Learning_Time_Series_EHR 
    - complex architecture, for general use
    - MIMIC and eICU dataset are very well suited but only accessible with 4-hours course
...
- **CA-GAN**: 
    ...
    - can ensure fairness for discrimination caused by a lack of data 
    - very bad implementation, no modules or functions, focus on representation bias

- **HiMoE**: 
    - no implementation could be found, maybe ask authors
    - complex architecture, maybe only for large datasets
    - use traffic data, fairness between nodes should be ensured
    - seems to focus on consistent predictions

- **BAHT**: 
    - exact search or ask authors, they stated they wanted to publish it but nothing could be found
...
- **FairFor**:
    ...
    - must be adjusted for group fairness
    - PeMS: traffic data of california, registration necessary
    - Fairness between regions is ensured
...
- **Absolute Correlation Regularization**: 
    ...
    - enhanced trip data with social average features for regions, groups regions, processed dataset is not available in repo
    - seems to focus on group fairness

- **Long Term Fairness**: 
    - https://github.com/yaoweihu/Achieving-Long-term-Fairness/tree/main 
    - only applicable on one bias generating attribute, considers long term effects and short term effects for group fairness, how much it focuses on group fairness is not clear 
...
- **FairTP**: 
    - private repo of jiangnanx129, maybe ask him
    - focused on traffic data
    - HK Didi and SD Dataset: sourced from Hong Kong Government and PeMS, traffic data, grouped sensor regions which is biased generating attribute 
...
- **Fairness criteria for LDS**: 
    ...
    - synthetic data and COMPAS (typical fairness dataset), not time series, LDS somehow transformed it to a time series problem, how they transformed it is not clear with first view at repo, 
    - Einbindung von kommerzieller Software Mosek 
